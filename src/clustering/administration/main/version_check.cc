...
void version_checker_t::periodic_checkin() {
    ...
    opts.form_data["Number-Of-Servers"] = metadata.machines.machines.size();
    opts.form_data["Uname"] = uname_msr();
    int tables = 0;
    {
        auto it = metadata.rdb_namespaces.namespaces.iterator();
        for (; it != metadata.rdb_namespaces.namespaces.end(); ++it) {
            auto pair = *it;
            tables++;
        }
    }
